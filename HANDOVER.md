...
- **Step 5 (schedule) DONE:** `daily.yml` runs **every day** at `cron: "17 6 * * *"`
  (~08:17 Paris) AND keeps the manual `workflow_dispatch` button. Search + image run
  ...
  NEVER fired it (it drops top-of-hour schedules under load); moved off the hour to `:17`.
  Scheduled runs DO publish (DRY_RUN defaults to false for `schedule`); only the manual
  button defaults to a dry run.
